...
void loop()
{
  bri_val = 99;    //analogRead(bri_pot_pin);    // read the brightness pot
  int sensorValue = analogRead(A0);
    // Convert the analog reading (which goes from 0 - 1023) to a voltage (0 - 5V):
    float voltage = sensorValue * (5.0 / 1023.0);
    float hue_val = -715 + voltage*1000;                  //   <--------NEED TO FIX SCALING FACTOR  
  
  Serial.print("pin 0 val "); Serial.print(voltage);
  Serial.print("      brightness "); Serial.print(bri_val); Serial.print("     hue "); Serial.println(hue_val);
